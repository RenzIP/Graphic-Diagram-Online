...
function Button($$renderer, $$props) {
  ...
  const baseStyles = "inline-flex items-center justify-center rounded-xl font-medium transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 disabled:pointer-events-none disabled:opacity-50 cursor-pointer no-underline active:scale-[0.98]";
  const variants = {
  ...
  if (href) {
    ...
    $$renderer.push(`<a${attributes({
      href,
      class: `${stringify(baseStyles)} ${stringify(variants[variant])} ${stringify(sizes[size])} ${stringify(className)}`,
      ...rest
    })}>`);
    children?.($$renderer);
    $$renderer.push(`<!----></a>`);
