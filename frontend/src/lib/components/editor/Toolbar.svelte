<script lang="ts">
	...
	import { exportPNG, exportSVG, exportJSON, exportDSL } from '$lib/utils/export';
	...
	function handleExport(format: string) {
		...
		switch (format) {
			case 'png':
				if (svgRef) exportPNG(svgRef, `${titleInput}.png`);
				break;
			case 'svg':
				if (svgRef) exportSVG(svgRef, `${titleInput}.svg`);
				break;
			case 'json':
